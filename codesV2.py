...
def Augment_anImage(coco,img_id=None,  N_augmented_obj=5, img_ids_to_augment_from=None,
                    bbox_kind = 'obbox',  plot_image=True, with_segment=True, with_category=True, plot_DAG=True, IoU=False, smoothing_factor = 0.25, alpha = 0.5):
  ...
  if smoothing_factor>0:
    ...
    for i in range(1,Mask_layers.shape[0]-1):
      Image_layers[i], Mask_layers[i] = refine_pasted_object(Image_layers[i], Mask_layers[i], I_destination, brush_size = smoothing_factor, alpha =alpha )
...
def refine_pasted_object(object_img, mask, bg_img, brush_size = 1, alpha = 0.5):
    """
    ...
    matched_object = alpha*matched_object + (1-alpha)*object_img
